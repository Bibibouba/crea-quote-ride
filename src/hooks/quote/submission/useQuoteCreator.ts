...
export const useQuoteCreator = () => {
  ...
  const createQuote = async (props: CreateQuoteProps): Promise<Quote> => {
    ...
    try {
      ...
      // Ajouter les coordonnées séparément pour éviter les problèmes de typage
      if (departureCoordinates) {
        quoteData.departure_coordinates = departureCoordinates;
      }
      if (destinationCoordinates) {
        quoteData.arrival_coordinates = destinationCoordinates;
      }
      if (customReturnCoordinates) {
        quoteData.return_coordinates = customReturnCoordinates;
      }
